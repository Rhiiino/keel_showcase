// keel_web/src/modules/coak/components/tags/CoakTagListRow.tsx

import { useEffect, useRef, useState, type RefObject } from "react";

import { useConfirmDeleteAction } from "../../../../hooks/useConfirmDeleteAction";
import type { CoakTag } from "../../api";
import { normalizeHexColor } from "../../lib/coakTagDisplay";
import { CardMenu } from "../../../../components/CardMenu";

export const COAK_TAG_LIST_TABLE_WIDTH_CLASS = "w-full";

export const COAK_TAG_LIST_GRID_CLASS =
  "grid w-full grid-cols-[4.5rem_minmax(0,1fr)_minmax(0,1.25fr)_3rem_2.5rem] items-center";

type CoakTagListRowProps = {
  tag: CoakTag;
  disabled?: boolean;
...
export function CoakTagListRow({
  tag,
  disabled = false,
  onRename,
  onDescriptionChange,
  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 items-center px-4 py-2.5">
        <input
          ref={inputRef}
          type="text"
          value={draftName}
          ...
          className="w-full truncate rounded-lg bg-transparent px-2 py-1.5 text-sm text-stone-100 ring-1 ring-transparent transition focus:bg-stone-900/50 focus:outline-none focus:ring-stone-700 disabled:opacity-50"
      ...
      <div className="flex min-w-0 items-center px-4 py-2.5">
        <input
          ...
          className="w-full truncate rounded-lg bg-transparent px-2 py-1.5 text-sm text-stone-100 placeholder:text-stone-600 ring-1 ring-transparent transition focus:bg-stone-900/50 focus:outline-none focus:ring-stone-700 disabled:opacity-50"
          aria-label={`Tag description for ${tag.name}`}
        />
      </div>

      <div className="flex items-center justify-center px-4 py-2.5">
        <span className="text-sm tabular-nums text-stone-300">{tag.item_count}</span>
      </div>

      <div
        ref={containerRef}
        className="relative z-20 flex items-center justify-center px-1 py-2.5"
      >
        {onDelete ? (
          <CardMenu
...
export function CoakTagDraftRow({
  name,
  colorHex,
  disabled = false,
  inputRef,
    ...
    >
      ...
      <div className="flex min-w-0 items-center px-4 py-2.5">
        <input
          ...
          className="w-full truncate rounded-lg bg-transparent px-2 py-1.5 text-sm text-stone-100 ring-1 ring-sky-500/40 transition focus:bg-stone-900/50 focus:outline-none focus:ring-sky-500/60 disabled:opacity-50"
