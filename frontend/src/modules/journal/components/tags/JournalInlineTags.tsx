// keel_web/src/modules/journal/components/tags/JournalInlineTags.tsx

import { useQuery } from "@tanstack/react-query";
import { useEffect, useMemo, useRef, useState } from "react";

import { fetchJournalTags, journalQueryKeys, type JournalTag } from "../../api";
import { journalTagPillStyle } from "../../lib/journalTagDisplay";

type JournalInlineTagsProps = {
  tagIdsDraft: number[];
...
export function JournalInlineTags({
  tagIdsDraft,
  onTagIdsDraftChange,
  disabled = false,
}: JournalInlineTagsProps) {
  ...
  const selectedTags = tagIdsDraft
    ...
    .filter((tag): tag is JournalTag => tag !== undefined);
            ...
            {!tagsQuery.isLoading && unselectedTags.length === 0 ? (
              <p className="px-3 py-2 text-xs text-stone-500">
                {allTags.length === 0
                  ...
                  : "All tags are already on this entry."}
              </p>
            ) : null}
