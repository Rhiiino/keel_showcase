// keel_web/src/modules/journal/components/tags/JournalTagListRow.tsx
...
import { normalizeHexColor } from "../../lib/journalTagDisplay";
import { JournalTagPill } from "./JournalTagPill";

export const JOURNAL_TAG_LIST_TABLE_WIDTH_CLASS = "w-full min-w-[44rem]";

export const JOURNAL_TAG_LIST_GRID_CLASS =
  "grid w-full grid-cols-[4.5rem_minmax(0,1fr)_minmax(0,14rem)_5rem_3.5rem] items-center";

type JournalTagListRowProps = {
  tag: JournalTag;
  disabled?: boolean;
...
export function JournalTagListRow({
  tag,
  disabled = false,
  onRename,
  onColorChange,
  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 items-center px-4 py-3.5">
        <JournalTagPill tag={previewTag} />
      </div>

      <div className="flex items-center justify-center px-4 py-3.5">
        <span className="text-sm tabular-nums text-stone-300">{tag.entry_count}</span>
      </div>
...
export function JournalTagDraftRow({
  name,
  colorHex,
  disabled = false,
  inputRef,
