// keel_web/src/modules/media/components/EntityMediaCarousel.tsx

// Horizontal media strip for entity forms (timeline/journal gallery parity).
...
export function EntityMediaCarousel({
  media,
  pendingUploads = [],
  ...
  const entries: CarouselEntry[] = useMemo(() => {
    const saved: CarouselEntry[] = media.map((item) => ({ kind: "media", media: item }));
    const pending: CarouselEntry[] = pendingUploads.map((item) => ({
      kind: "pending",
      pending: item,
    }));
        ...
        <div className="relative" {...dropZoneHandlers}>
          ...
          >
            ...
            {(onQueueUploads || onQueueMediaSelections) && (
              <EntityMediaAddCard disabled={disabled} onClick={openSourceDialog} />
            )}
          </div>
        </div>
      )}
