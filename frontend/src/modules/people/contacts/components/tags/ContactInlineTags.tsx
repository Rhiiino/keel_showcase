// keel_web/src/modules/contacts/components/tags/ContactInlineTags.tsx

import { useQuery } from "@tanstack/react-query";
import { useEffect, useMemo, useRef, useState } from "react";

import { contactsQueryKeys, fetchContactTags, type ContactTag } from "../../api";
import { contactTagPillStyle } from "../../lib/contactTagDisplay";

type ContactInlineTagsProps = {
  tagIdsDraft: number[];
...
export function ContactInlineTags({
  tagIdsDraft,
  onTagIdsDraftChange,
  disabled = false,
}: ContactInlineTagsProps) {
  ...
  const selectedTags = tagIdsDraft
    ...
    .filter((tag): tag is ContactTag => tag !== undefined);
  ...
  return (
    <div ref={containerRef} className="relative mt-2 flex flex-wrap items-center gap-2">
