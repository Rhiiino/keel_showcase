// keel_web/src/modules/contacts/components/tags/ContactTagListRow.tsx
...
import { normalizeHexColor } from "../../lib/contactTagDisplay";
import { ContactTagPill } from "./ContactTagPill";

export const CONTACT_TAG_LIST_TABLE_WIDTH_CLASS = "w-full min-w-[44rem]";

export const CONTACT_TAG_LIST_GRID_CLASS =
  "grid w-full grid-cols-[4.5rem_minmax(0,1fr)_minmax(0,14rem)_5rem_3.5rem] items-center";

type ContactTagListRowProps = {
  tag: ContactTag;
  disabled?: boolean;
...
export function ContactTagListRow({
  tag,
  disabled = false,
  onRename,
  onColorChange,
  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 items-center px-4 py-3.5">
        <ContactTagPill tag={previewTag} />
      </div>

      <div className="flex items-center justify-center px-4 py-3.5">
        <span className="text-sm tabular-nums text-stone-300">{tag.contact_count}</span>
      </div>
...
export function ContactTagDraftRow({
  name,
  colorHex,
  disabled = false,
  inputRef,
