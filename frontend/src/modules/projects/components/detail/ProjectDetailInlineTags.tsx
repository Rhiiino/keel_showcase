// stack_sandbox/frontend_web/src/modules/projects/components/detail/ProjectDetailInlineTags.tsx

// Draft tag row for the project detail display view (saved via header Save).

import { useQuery } from "@tanstack/react-query";
import { useEffect, useMemo, useRef, useState } from "react";

import { fetchProjectTags, projectsQueryKeys, type ProjectTag } from "../../api";
import { projectTagPillStyle } from "../../lib/project";

type ProjectDetailInlineTagsProps = {
  tagIdsDraft: number[];
...
export function ProjectDetailInlineTags({
  tagIdsDraft,
  onTagIdsDraftChange,
  disabled = false,
}: ProjectDetailInlineTagsProps) {
  ...
  return (
    ...
    >
      <div
        className="pointer-events-none absolute inset-x-0 top-0 border-t border-stone-800/60 lg:right-10"
        aria-hidden
      />
          ...
          <div className="absolute left-0 top-full z-30 mt-2 min-w-[12rem] overflow-hidden rounded-lg border border-stone-800 bg-stone-950 py-1 shadow-lg ring-1 ring-stone-800/80">
            ...
            {!tagsQuery.isLoading && unselectedTags.length === 0 && (
              <p className="px-3 py-2 text-xs text-stone-500">
                {allTags.length === 0
                  ? "No tags yet. Create tags on the Tags page."
                  : "All tags are already on this project."}
              </p>
            )}
