// keel_web/src/modules/projects/components/tags/ProjectTagListRow.tsx
...
import { normalizeHexColor } from "../../lib/project";
import { ProjectTagPill } from "./ProjectTagPill";

export const PROJECT_TAG_LIST_TABLE_WIDTH_CLASS = "w-full min-w-[56rem]";

export const PROJECT_TAG_LIST_GRID_CLASS =
  "grid w-full grid-cols-[4.5rem_minmax(0,1fr)_minmax(0,1.25fr)_minmax(0,14rem)_5rem_3.5rem] items-center";

type ProjectTagListRowProps = {
  tag: ProjectTag;
  disabled?: boolean;
...
export function ProjectTagListRow({
  tag,
  disabled = false,
  onRename,
  onDescriptionChange,
  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 items-center px-4 py-3.5">
        <ProjectTagPill tag={previewTag} />
      </div>

      <div className="flex items-center justify-center px-4 py-3.5">
        <span className="text-sm tabular-nums text-stone-300">{tag.project_count}</span>
      </div>
...
export function ProjectTagDraftRow({
  name,
  colorHex,
  disabled = false,
  inputRef,
